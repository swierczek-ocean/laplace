function ftimevec = wfnClenshawSigmab(NLag,TimeInput,sigmaP,bP,LaguerreCoef)
%WFNCLENSHAWSIGMAB Laguerre polynomial evaluation 
...
%  
%  Use:
%  ftimevec = wfnClenshawSigmab(NLag,TimeInput,sigmaP,bP,LaguerreCoef)
%  
%  Input:
%  NLag = number of Laguerre polynomial coefficients
%  TimeInput = input time (scalar or a column vector)
%  sigmaP = Weeks' method parameter sigma
%  bP = Week's method parameter b
...
for kidx=(NLag-1):-1:1
 Cpresent = ((2*kidx-1-(2*bP*TimeInput))/kidx).*Cpone - (kidx/(kidx+1)).*Cptwo + LaguerreCoef(kidx)*ones(Ntimes,1);
 Cptwo = Cpone; 
 Cpone = Cpresent;  
end
 
ftimevec = exp((sigmaP-bP)*TimeInput).*Cpresent;
